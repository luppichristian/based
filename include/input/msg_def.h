...
typedef enum msg_type {
  ...
  MSG_TYPE_OBJECT_LIFECYCLE = MSG_TYPE_USER + 1,
  MSG_TYPE_THREAD_CTX = MSG_TYPE_USER + 2,
...
typedef enum msg_object_event_kind {
  MSG_OBJECT_EVENT_CREATE = 1,
  MSG_OBJECT_EVENT_DESTROY = 2,
} msg_object_event_kind;

typedef enum msg_object_type {
  MSG_OBJECT_TYPE_THREAD = 1,
  MSG_OBJECT_TYPE_THREAD_GROUP = 2,
  MSG_OBJECT_TYPE_MUTEX = 3,
  MSG_OBJECT_TYPE_CONDVAR = 4,
  MSG_OBJECT_TYPE_RWLOCK = 5,
  MSG_OBJECT_TYPE_SEMAPHORE = 6,
  MSG_OBJECT_TYPE_SPINLOCK = 7,
  MSG_OBJECT_TYPE_ARENA = 8,
  MSG_OBJECT_TYPE_HEAP = 9,
  MSG_OBJECT_TYPE_POOL = 10,
  MSG_OBJECT_TYPE_RING = 11,
  MSG_OBJECT_TYPE_PROCESS = 12,
  MSG_OBJECT_TYPE_ARCHIVE = 13,
  MSG_OBJECT_TYPE_PATHWATCH = 14,
  MSG_OBJECT_TYPE_LOG_STATE = 15,
...
typedef enum msg_thread_ctx_event_kind {
  MSG_THREAD_CTX_EVENT_INIT = 1,
  MSG_THREAD_CTX_EVENT_QUIT = 2,
} msg_thread_ctx_event_kind;

typedef enum msg_pathwatch_event_kind {
  MSG_PATHWATCH_EVENT_ITEM = 1,
...
typedef struct msg_object_lifecycle_data {
  u32 object_type;
  u32 event_kind;
  void* object_ptr;
} msg_object_lifecycle_data;

typedef struct msg_thread_ctx_data {
  u32 event_kind;
  u64 thread_id;
  void* ctx_ptr;
} msg_thread_ctx_data;

typedef struct msg_pathwatch_data {
  u32 event_kind;
  i64 pathwatch_id;
  i64 watch_id;
  u32 action;
...
typedef struct msg {
  ...
  union {
    ...
    msg_object_lifecycle_data object_lifecycle;
    msg_thread_ctx_data thread_ctx;
    msg_pathwatch_data pathwatch;
    msg_log_data log;
    msg_assert_data assert_data;
  };
} msg;
