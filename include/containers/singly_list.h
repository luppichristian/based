...
#define SINGLY_LIST_EMPTY(head, tail) expression((head) == nullptr)

// SINGLY_LIST_COUNT counts the number of nodes by traversing from head to end.
#define SINGLY_LIST_COUNT(head, tail, count) expression({ \
  (count) = 0;                                            \
  auto _node = (head);                                    \
  while (_node != nullptr) {                              \
    (count)++;                                            \
...
#define SINGLY_LIST_HEAD(head, tail) expression((head))

// SINGLY_LIST_TAIL returns the tail node of the list.
#define SINGLY_LIST_TAIL(head, tail) expression((tail))

// SINGLY_LIST_PUSH_FRONT adds a node to the front of the list.
#define SINGLY_LIST_PUSH_FRONT(head, tail, node) expression({ \
  (node)->next = (head);                                      \
...
#define SINGLY_LIST_PUSH_BACK(head, tail, node) expression({ \
  (node)->next = nullptr;                                    \
...
#define SINGLY_LIST_POP_FRONT(head, tail, node) expression({ \
  (node) = (head);                                           \
  if ((head) != nullptr) {                                   \
    (head) = (head)->next;                                   \
