...
func void ring_cpy_out(ring* rng, sz ring_offset, void* dst, sz byte_count) {
  profile_func_begin;
...
func void ring_cpy_in(ring* rng, sz ring_offset, void* src, sz byte_count) {
  profile_func_begin;
  sz to_end = rng->capacity - ring_offset;
  u8* src_bytes = (u8*)src;
  if (byte_count <= to_end) {
...
func sz ring_write(ring* rng, void* data, sz size) {
  ...
  if (write_sz > 0) {
    ring_cpy_in(rng, rng->write_pos, data, write_sz);
...
func sz ring_read(ring* rng, void* out, sz size) {
  ...
  if (read_sz > 0) {
    ring_cpy_out(rng, rng->read_pos, out, read_sz);
...
func sz ring_peek(ring* rng, void* out, sz size) {
  ...
  if (peek_sz > 0) {
    ring_cpy_out(rng, rng->read_pos, out, peek_sz);
  }
