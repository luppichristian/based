...
#define BINARY_TREE_NEXT_INORDER(root, node, out) stmt(                                     \
    type_of((root)) _binary_tree_root = (root);                                             \
    type_of((node)) _binary_tree_cursor = (node);                                           \
    (out) = NULL;                                                                           \
    if (_binary_tree_root != NULL && _binary_tree_cursor != NULL) {                         \
      if (_binary_tree_cursor->right != NULL) {                                             \
        ...
        safe_while (_binary_tree_cursor != _binary_tree_root) {                             \
          type_of((_binary_tree_cursor)) _binary_tree_parent = _binary_tree_cursor->parent; \
...
#define BINARY_TREE_ROTATE_LEFT(root_ptr, node) stmt( \
    type_of((node)->right) _right = (node)->right;    \
...
#define BINARY_TREE_ROTATE_RIGHT(root_ptr, node) stmt( \
    type_of((node)->left) _left = (node)->left;        \
...
#define BINARY_TREE_FOREACH_PREORDER(root, it)           \
  safe_for (type_of(((root))) it = (root); (it) != NULL; \
            (it) = ({ type_of((root)) _binary_tree_next = NULL; BINARY_TREE_NEXT_PREORDER((root), (it), _binary_tree_next); _binary_tree_next; }))

#define BINARY_TREE_FOREACH_INORDER(root, it) \
  safe_for (type_of(((root))) it = ({ type_of((root)) _binary_tree_first = NULL; BINARY_TREE_FIRST_INORDER((root), _binary_tree_first); _binary_tree_first; });     \
            (it) != NULL;                     \
            (it) = ({ type_of((root)) _binary_tree_next = NULL; BINARY_TREE_NEXT_INORDER((root), (it), _binary_tree_next); _binary_tree_next; }))

#define BINARY_TREE_FOREACH_POSTORDER(root, it) \
  safe_for (type_of(((root))) it = ({ type_of((root)) _binary_tree_first = NULL; BINARY_TREE_FIRST_POSTORDER((root), _binary_tree_first); _binary_tree_first; });       \
            (it) != NULL;                       \
            (it) = ({ type_of((root)) _binary_tree_next = NULL; BINARY_TREE_NEXT_POSTORDER((root), (it), _binary_tree_next); _binary_tree_next; }))
